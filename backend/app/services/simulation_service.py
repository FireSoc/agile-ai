...
def generate_inbox_preview(
    ...
    Rules
    -----
    - email_sent at day 0 for each task.
    - awaiting_reply at day 0 for customer-required tasks.
    ...
    for task in tasks:
        ...
        # Always send an outbound email at project day 0
        sent.append(_msg("email_sent", 0.0, "sent"))

        if task.is_customer_required:
            # Sender side: awaiting reply
            sent.append(_msg("awaiting_reply", 0.5, "sent"))
    ...
    for task in project.tasks:
        ...
        sim_tasks.append(
            SimulationTaskInput(
                ...
                delay_days=delay,
                # v2 fields: use safe defaults since ORM task model doesn't carry them yet
                criticality=2,
