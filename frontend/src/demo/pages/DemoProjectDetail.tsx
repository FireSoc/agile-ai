...
import { demoProjectsApi, demoCustomersApi, demoAiApi, demoQueryKeys } from '@/demo/demoApi';
...
export function DemoProjectDetail() {
  ...
  function handleCompanyChange(customerId: number) {
    ...
    if (targetId != null) navigate(DEMO_ROUTES.projectDetail(targetId));
  }

  function handleProjectChange(pid: number) {
    navigate(DEMO_ROUTES.projectDetail(pid));
  }
  ...
  const checkOverdueMutation = useMutation({
    mutationFn: () => demoProjectsApi.checkOverdue(projectId),
    onSuccess: (data) => {
      invalidate();
  ...
  const checkRiskMutation = useMutation({
    mutationFn: () => demoProjectsApi.checkRisk(projectId),
    onSuccess: (data) => {
      invalidate();
  ...
  const recalculateRiskMutation = useMutation({
    mutationFn: () => demoProjectsApi.recalculateRisk(projectId),
    onSuccess: () => invalidate(),
  });

  const advanceStageMutation = useMutation({
    mutationFn: () => demoProjectsApi.advanceStage(projectId),
    onSuccess: (data) => {
      invalidate();
  ...
  const { data: risk } = useQuery({
    queryKey: [...demoQueryKeys.project(projectId), 'risk'],
    queryFn: () => demoProjectsApi.getRisk(projectId),
    enabled: !!project && !isNaN(projectId),
  });

  const riskSummaryMutation = useMutation({
    mutationFn: () => demoAiApi.getRiskSummary(projectId),
  });
  ...
  useLayoutEffect(() => {
    ...
    setPageLayout({
      ...
      action: (
        <Link
          to={`${DEMO_ROUTES.simulator}?projectId=${project.id}&tab=compare`}
  ...
  const recommendations = (project.recommendations ?? []).filter((r) => !r.dismissed);
    ...
    <PageContainer className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center gap-4 overflow-visible">
        <div className="flex items-center gap-2">
          ...
          <Select value={String(project.customer_id)} onValueChange={(v) => handleCompanyChange(Number(v))}>
            <SelectTrigger id="project-detail-company" size="sm" className="w-48 overflow-visible" aria-label="Switch company">
              <SelectValue placeholder="Select company">{customer?.company_name}</SelectValue>
            </SelectTrigger>
            <SelectContent>
              {(customers ?? []).map((c) => (
        ...
        <Link to={DEMO_ROUTES.projects} className="text-sm font-medium text-muted-foreground underline-offset-4 hover:underline inline-flex items-center gap-1">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <section aria-labelledby="tasks-cta-heading" data-demo-target="project-task-list">
              ...
              <CardHeader className="flex flex-row items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    <span className="font-medium text-foreground">{completedTaskCount}/{tasks.length}</span> completed
                    {overdueTaskCount > 0 && (
                ...
                <Link to={DEMO_ROUTES.projectTasks(project.id)} className={cn(buttonVariants({ variant: 'default', size: 'sm' }), 'gap-1.5 shrink-0 inline-flex items-center')}>
                  Open task list
                </Link>
              </CardHeader>
            </Card>
            ...
            <Card>
              <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
                <CardTitle id="project-glance-heading" className="text-base">
                  ...
                  {project.risk_flag && <AlertTriangle className="ml-1.5 inline h-4 w-4 text-destructive" aria-label="At risk" />}
                </CardTitle>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground border-t border-border pt-3">
                  ...
                  {(project.target_go_live_date || project.projected_go_live_date) && (
                    <span className="flex items-center gap-1.5">
                      ...
                      Go-live: {project.projected_go_live_date ? new Date(project.projected_go_live_date).toLocaleDateString('en-US') : project.target_go_live_date ? new Date(project.target_go_live_date).toLocaleDateString('en-US') : '—'}
                    </span>
                  )}
                  <span className="flex items-center gap-1.5">
                ...
                {project.notes && <p className="text-sm text-muted-foreground italic border-t border-border pt-3">{project.notes}</p>}
              </CardContent>
            </Card>
          </section>
          ...
          <section aria-labelledby="ai-risk-summary-heading">
            <Card aria-labelledby="ai-risk-summary-heading">
              ...
              <CardContent className="space-y-3">
                ...
                >
                  {riskSummaryMutation.isPending ? <LoadingSpinner size="sm" /> : <Sparkles className="h-3.5 w-3.5" />}
                  Generate
                </Button>
                {riskSummaryMutation.isError && <p className="text-sm text-destructive">Could not load.</p>}
        ...
        <div className="space-y-6">
          {(project.risk_flag || risk != null || (riskSignals?.length ?? 0) > 0) && (
            <Card className="border-primary/20" aria-labelledby="risk-heading" data-demo-target="project-risk">
              <CardHeader className="pb-2">
              ...
              <CardContent className="space-y-2">
                <div className="flex items-center gap-2">
                  <RiskGauge score={risk?.risk_score ?? project.risk_score ?? null} className="max-w-[6rem] flex-1" />
                  <RiskScoreBadge score={risk?.risk_score ?? project.risk_score ?? null} level={risk?.risk_level ?? project.risk_level ?? undefined} showScore={true} />
                </div>
                {((risk?.explanations?.length ?? 0) > 0 || (riskSignals?.length ?? 0) > 0) && (
                  <ul className="text-xs text-foreground list-disc list-inside space-y-0.5">
                    ...
                    {((risk?.explanations?.length ?? 0) + (riskSignals?.length ?? 0)) > 3 && <li className="text-muted-foreground">…</li>}
                  </ul>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border-amber-200 bg-amber-50/30" aria-labelledby="blockers-heading">
              ...
              <CardContent>
                <ul className="text-xs text-foreground space-y-1">
                  {blockedTasks.map((t) => (
                    <li key={t.id}><span className="font-medium">{t.title}</span>{t.blocker_reason && ` — ${t.blocker_reason}`}</li>
                  ))}
                </ul>
              </CardContent>
            </Card>
          ...
          {(project.next_best_action || recommendations.length > 0) && (
            <Card aria-labelledby="next-heading">
              ...
              <CardContent>
                {project.next_best_action && <p className="text-xs text-foreground">{project.next_best_action}</p>}
          ...
          <Card aria-labelledby="sidebar-actions">
            ...
            <CardContent className="flex flex-wrap gap-2">
              ...
              <Button variant={project.risk_flag ? 'destructive' : 'secondary'} size="sm" onClick={() => checkRiskMutation.mutate()} disabled={checkRiskMutation.isPending}>
          ...
          <Card aria-labelledby="events-heading">
            <CardHeader className="flex flex-row items-center gap-2 border-b pb-2">
              ...
              <CardTitle id="events-heading" className="text-sm">Activity</CardTitle>
              {events.length > 0 && <ShadcnBadge variant="secondary" className="ml-auto font-normal text-xs">{events.length}</ShadcnBadge>}
            </CardHeader>
